...
import { X, Camera, Check, ChevronRight, Search, MoreVertical, ChevronLeft, Pencil, MessageCircle, ThumbsUp } from 'lucide-react';
...
export default function App() {
  const [currentView, setCurrentView] = useState<'board' | 'write'>('board');
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#111111] text-[#E0E0E0] font-sans mx-auto max-w-md w-full relative overflow-hidden">
      {currentView === 'board' ? (
        <BoardView onWriteClick={() => setCurrentView('write')} posts={posts} />
      ) : (
...
function BoardView({ onWriteClick, posts }: { onWriteClick: () => void, posts: Post[] }) {
  return (
    <div className="flex flex-col h-full relative">
      ...
      <main className="flex-1 overflow-y-auto pb-[100px] scrollbar-hide">
        ...
        <div className="flex flex-col">
          {posts.map(post => (
            <div key={post.id} className="p-4 border-b border-[#2A2A2A] active:bg-[#1a1a1a] transition-colors cursor-pointer">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 px-4">
          <div className="bg-[#1c1c1c] w-[300px] rounded-[14px] overflow-hidden flex flex-col shadow-2xl">
            <div className="pt-6 pb-5 px-5 text-center flex flex-col items-center">
              <p className="text-[16px] font-bold text-[#E0E0E0] mb-3">익명 글 작성 유의사항</p>
            ...
            <div className="flex border-t border-[#333333]">
              ...
              <button 
                className="flex-1 py-3.5 text-[15px] text-[#c62917] font-bold active:bg-[#2A2A2A] transition-colors"
                onClick={handleAnonymousWarningSubmit}
              >
                확인
              </button>
            </div>
